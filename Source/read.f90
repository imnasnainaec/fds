...
INTEGER :: IJK(3),NM,NM2,CURRENT_MPI_PROCESS,MPI_PROCESS,RGB(3),LEVEL,N_MESH_NEW,N,II,JJ,KK,NMESHES_READ,NNN,NEVAC_MESHES, &
           NMESHES_EVAC, NMESHES_FIRE, NM_EVAC, N_THREADS, QUOTIENT, REMAINDER
      ...
      J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
         I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
            ...
            IF (MPI_PROCESS>-1) THEN
               ...
               ENDIF
            ELSE
               ! One-to-one assignment of meshes to MPI processes; 
               !  imbalances if more meshes than processes:
               !CURRENT_MPI_PROCESS = MIN(NM-1,N_MPI_PROCESSES-1)

               ! Block assignment of meshes to MPI processes:
               QUOTIENT = NMESHES / N_MPI_PROCESSES
               REMAINDER = MODULO(NMESHES, N_MPI_PROCESSES)
               IF (NM<=REMAINDER*(QUOTIENT + 1)) THEN
                   CURRENT_MPI_PROCESS = (NM - 1)/(QUOTIENT + 1)
               ELSE
                   CURRENT_MPI_PROCESS = (NM - REMAINDER - 1)/QUOTIENT
               ENDIF
            ENDIF
...
NAMELIST /MISC/ AEROSOL_SCRUBBING, AGGLOMERATION,AEROSOL_AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,&
                ...
                IBLANK_SMV,MAX_LEAK_PATHS,MAXIMUM_VISIBILITY,MPI_TIMEOUT,&
...
NAMELIST /VENT/ COLOR,CTRL_ID,DB,DEVC_ID,DYNAMIC_PRESSURE,EVACUATION,FYI,ID,IOR,L_EDDY,L_EDDY_IJ,MB,MESH_ID,MULT_ID,N_EDDY,OBST_ID,&
...
NAMELIST /DEVC/ ABSOLUTE_VALUE,BYPASS_FLOWRATE,CONVERSION_FACTOR,CONVERSION_ADDEND,COORD_FACTOR,CELL_L,CTRL_ID,DB,DELAY,DEPTH,&
...
REAL(EB) :: AGL_SLICE
INTEGER :: N,NN,NM,MESH_NUMBER,N_SLCF_O,NITER,ITER,VELO_INDEX,IOR,IG,TRNF_INDEX,GEOM_INDEX
...
NAMELIST /SLCF/ AGL_SLICE,CELL_CENTERED,DB,EVACUATION,FACE_CENTERED,FIRE_LINE,FYI,ID,IOR,LEVEL_SET_FIRE_LINE,MAXIMUM_VALUE,&
                MATL_ID,MESH_NUMBER,MINIMUM_VALUE,PART_ID,PBX,PBY,PBZ,PROP_ID,QUANTITY,QUANTITY2,REAC_ID,SLICETYPE,SMV_OFFSET,&
...
LOGICAL :: PER_MESH
INTEGER :: NM
NAMELIST /CSVF/ CSVFILE,UVWFILE,PER_MESH
